...
import { getFavoriteMoviesFromApi } from '../API/TMDBApi'
...
class Favorites extends React.Component {
    ...
    _getFavoriteMovies() {
        ...
        const action = { type: "CLEAR_FAVORITE_MOVIES", value: {} }
        this.props.dispatch(action)
        getFavoriteMoviesFromApi(account_id, session_id, 1).then(data => {
            data.results.forEach(element => {
                const action = { type: "TOGGLE_FAVORITE", value: element }
                this.props.dispatch(action)
            });
            for( var i = 2 ; i <= data.total_pages ; i++){
                getWatchListFromApi(account_id, session_id, i).then(data => {
                    const action = { type: "TOGGLE_FAVORITE", value: data }
                    this.props.dispatch(action)
                })
            }
            ...
            if(this.state.isLoading === false)
            {
                return (
                    <View style={styles.main_container_alert}>
                        <Text style={styles.bigTitle}>Favorites</Text>
                        <Text style={styles.text}>You haven't added any movies to your favorites list.</Text>
                        <TouchableOpacity onPress={() => this._getFavoriteMovies()}>
